...
- **`app/api/submit-lead/route.ts`**: webhook + Google Sheets (both soft-fail; succeeds if either works).
- **Do not** force-redirect `/api/submit-lead` to the Netlify function — that skips Sheets.
- **`netlify/functions/submit-lead.js`**: webhook-only fallback for `netlify dev` if needed.

Set Google Sheets env vars with scope **Functions** (and Builds) in Netlify, not Build-only.
...
## This project (Family Court Accountant)

| Setting | Value |
|---------|--------|
| **`BRAND_NAME`** | `FamilyCourtAccountant` |
| **`domain`** (when `NEXT_PUBLIC_SITE_URL` is set) | `familycourtaccountant.com` |
| Contact webhook | **`/api/submit-lead`** on form submit |
| Google Sheets | Same handler — written before webhook; succeeds if either works |
